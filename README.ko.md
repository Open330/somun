...
**홍보가 서툰 개발자를 위해, 실제 작업물을 사용자가 이해하는 소개 글로 바꿔 줍니다.**

<sub>써 보기: <a href="https://somun.jiun.dev">somun.jiun.dev</a></sub>
...
<img src="docs/demo.gif" alt="소문: 글감을 고르고, 채널별 초안과 린트 결과를 확인하고, 복사한 뒤 링크 미리보기를 점검하는 화면" width="860" />

<sub>글감 → 채널별 초안과 점검 결과 → 복사 → 링크 미리보기 점검. 공개 저장소(Open330)로 녹화했습니다.</sub>

</div>

<br />

만든 건 많은데, 설명하기가 어렵습니다.

소문은 릴리스·PR·커밋을 읽고, 정말 알릴 만한 것이 생겼을 때 사용자가 알아듣는 말로 채널별 초안을 씁니다. X, Threads, LinkedIn, Show HN, GeekNews. 근거는 실제 작업물에서만 가져옵니다. 검수하고 복사해서 직접 올리는 건 사용자 몫입니다. 고친 문장은 다음 초안의 문체가 되고, 올린 글의 URL을 등록하면 그때부터 지표가 붙습니다.
...
화면은 한국어와 영어를 지원합니다. 브라우저 언어를 따르고 사이드바에서 바꿀 수 있습니다. 고른 언어는 판단 이유, 오류 안내, 주간 알림의 언어도 정합니다.
...
로컬 에이전트 모드는 모든 모델 호출(다이제스트, 판단, 초안, 저장소 프로필, 문체 규칙)을 작업 큐에 넣습니다. 원자료가 서버 모델로 가지 않습니다. CLI가 로그인된 컴퓨터에서 워커를 켭니다.
