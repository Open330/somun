  ...
  try {
    ...
    for (let n = 0; n < 2; n++) {
      ...
      try {
        ...
        expect(db.$client.prepare("SELECT count(*) AS n FROM __drizzle_migrations").get()).toEqual({ n: 9 });
...
it("keeps the migration schema aligned with the latest generation snapshot", () => {
  ...
  const snapshot = JSON.parse(readFileSync("drizzle/meta/0008_snapshot.json", "utf8")) as { tables: Record<string, { columns: Record<string, unknown> }> };
  try {
