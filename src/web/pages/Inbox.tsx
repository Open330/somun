...
export default function Inbox() {
  ...
  async function collect() {
    ...
    try {
      ...
      setMessage({ text: failed ? t("일부 소스를 확인하지 못했습니다. 연결 상태를 확인한 뒤 다시 시도해 주세요.") : count ? t("새 변경 {n}건을 가져왔습니다. 아래에서 글감을 골라보세요.", { n: count }) : t("확인을 마쳤습니다. 새로 가져올 변경은 없습니다."), error: failed });
      reload(); reloadSources();
    } catch (err) { setMessage({ text: `${t("변경을 가져오지 못했습니다.")} ${(err as Error).message}`, error: true }); }
  ...
  async function judge(id: number) {
    ...
    try {
      ...
      setMessage({ text: t("초안 준비를 요청했습니다. 결과가 도착하면 목록이 갱신됩니다. 오래 기다리면 글감을 열어 상태를 확인해 주세요.") });
      reload();
    } catch (err) { setMessage({ text: `${t("요청하지 못했습니다.")} ${(err as Error).message}`, error: true }); }
    finally { setRequested((ids) => ids.filter((value) => value !== id)); }
  }

  if (error || sourceError) return <ErrorState title={t("글감을 불러오지 못했습니다")} message={error ?? sourceError!} onRetry={() => { reload(); reloadSources(); }} />;
  ...
  return (
    <>
      ...
      {firstUse ? <>
        ...
        {collectable.length > 0 && <div className="next-action"><div><h2>{t("연결 준비가 끝났어요")}</h2><p>{t("최근 변경을 가져와 첫 글감을 찾아보세요.")}</p></div><button className="primary" disabled={busy} onClick={() => void collect()}>{busy ? t("변경 가져오는 중…") : t("첫 글감 가져오기 →")}</button></div>}
      </> : <>
        <div className="inbox-controls"><div className="filter-strip" role="group" aria-label={t("글감 상태 필터")}>{FILTERS.map(([key, label]) => <button key={key} aria-pressed={filter === key} className={filter === key ? "active" : ""} onClick={() => setFilter(key)}>{t(label)}<span>{counts[key]}</span></button>)}</div><label className="search-field"><span className="sr-only">{t("글감 제목 또는 저장소 검색")}</span><input type="search" placeholder={t("제목 또는 저장소 검색")} value={query} onChange={(event) => setQuery(event.target.value)} /></label></div>
        <div className="list-meta"><span>{t("{n}개 글감", { n: visible.length })}</span><span>{settings?.watch.mode === "auto" ? t("자동 준비 켜짐") : t("선택한 글감만 초안 준비")}{latestPoll ? ` · ${t("마지막 수집 {when}", { when: relTime(latestPoll) })}` : ""}</span></div>
        {visible.length === 0 ? <div className="state-panel"><span className="state-symbol" aria-hidden>⌕</span><h2>{query ? t("검색 결과가 없어요") : t("이 상태의 글감은 아직 없어요")}</h2><p>{query ? t("다른 검색어를 입력하거나 필터를 초기화해 보세요.") : t("다른 글감을 살펴보거나 새 변경을 가져와 보세요.")}</p><button onClick={() => { setQuery(""); setFilter("all"); }}>{t("전체 글감 보기")}</button></div> : (
          (filter === "all" ? FILTERS.filter(([key]) => !["all", "archived"].includes(key)) : FILTERS.filter(([key]) => key === filter)).map(([key, label]) => {
            ...
            return <Section key={key} title={t(label)} count={items.length}><div className="story-list">{items.map((c) => <article className="story-row" key={c.id}>
              <div className="story-content"><div className="story-meta"><span>{c.repo}</span><span>·</span><span>{typeLabel(c.type)}</span><span>· {relTime(c.updatedAt)}</span></div><Link className="story-title" to={`/c/${c.id}`}>{c.title}</Link><p>{c.judgment?.reasoning.split("\n")[0] || c.evidence.highlights?.[0] || t("이 변화에서 알릴 만한 내용을 찾아 초안으로 정리할 수 있어요.")}</p></div>
              <div className="story-actions"><StageChip stage={stageOf(c)} />{c.judgment && <span className="story-score" title={t("설정한 판단 기준의 가중 합계")}>{t("추천점수 {n}", { n: c.judgment.total })}</span>}<div className="toolbar">{category(c) === "fresh" && <button className="sm" disabled={requested.includes(c.id)} onClick={() => void judge(c.id)}>{requested.includes(c.id) ? t("요청 중…") : t("초안 준비")}</button>}<Link className="btn sm" to={`/c/${c.id}`}>{category(c) === "review" ? t("초안 검토") : t("자세히 보기")}<span aria-hidden> →</span></Link></div></div>
            </article>)}</div></Section>;
          })
        )}
      </>}
