...
export default function Pick() {
  ...
  useEffect(() => {
    if (!instId) { setErr("installation_id가 없습니다."); return; }
  ...
  async function save() {
    ...
    try {
      ...
      showToast(`${r.count}개를 지켜봅니다. 첫 확인을 시작했습니다.`);
      setTimeout(() => nav("/", { replace: true }), 900);
    } catch (e) { showToast(`저장 실패: ${(e as Error).message}`); } finally { setBusy(false); }
  }
  ...
  return (
    <>
      <div className="page-head">
        <div>
          <h1>무엇을 만들어볼까요?</h1>
          <p className="lede">지켜볼 저장소를 고르세요. 고른 것만 읽고, 아래 방식대로 글감을 만듭니다. 나중에 <a href="/connectors">연결</a>에서 바꿀 수 있습니다.</p>
        </div>
        <div className="toolbar">
          <button className="primary" disabled={busy || sel.size === 0 || !repos} onClick={() => void save()}>{busy ? "저장 중…" : `${sel.size}개 지켜보기`}</button>
        </div>
      </div>
        ...
        <>
          <div className="card" style={{ marginBottom: 14 }}>
            <div className="row" style={{ gap: 10, flexWrap: "wrap", alignItems: "center" }}>
              <b className="small">글감 처리</b>
              <label className="row small" style={{ gap: 6 }}><input type="radio" name="mode" checked={auto === false} onChange={() => setAuto(false)} /> 모아두고 내가 고른 것만 판단</label>
              <label className="row small" style={{ gap: 6 }}><input type="radio" name="mode" checked={auto === true} onChange={() => setAuto(true)} /> 자동: 최근 {settings?.watch.recentDays ?? 30}일 안에 생긴 글감은 바로 판단·초안</label>
            </div>
            <p className="tiny muted" style={{ marginTop: 6 }}>자동은 모델 호출이 많아집니다. 처음엔 수동으로 두고 어떤 글감이 오는지 본 뒤 켜는 편이 낫습니다.</p>
          </div>

          <div className="row" style={{ gap: 8, flexWrap: "wrap", alignItems: "center", marginBottom: 10 }}>
            <input placeholder="검색" value={q} onChange={(ev) => setQ(ev.target.value)} style={{ width: 200 }} />
            <div className="tabs" style={{ margin: 0 }}>
              {([[7, "7일"], [30, "30일"], [90, "90일"], [0, "전체"]] as [Recent, string][]).map(([d, l]) => <button key={d} className={`sm ${recent === d ? "active" : ""}`} onClick={() => setRecent(d)}>{d ? `최근 ${l} 갱신` : l}</button>)}
            </div>
            <label className="row small" style={{ gap: 6 }}>스타 ≥ <input type="number" min={0} value={minStars} onChange={(ev) => setMinStars(Number(ev.target.value) || 0)} style={{ width: 64 }} /></label>
            <label className="row small" style={{ gap: 6 }}><input type="checkbox" checked={hideForks} onChange={(ev) => setHideForks(ev.target.checked)} /> 포크 제외</label>
            <label className="row small" style={{ gap: 6 }}><input type="checkbox" checked={hideArchived} onChange={(ev) => setHideArchived(ev.target.checked)} /> 아카이브 제외</label>
            <span className="tiny muted">{visible.length}/{repos.length}개 표시 · {sel.size}개 선택</span>
            <button className="ghost sm" onClick={() => setSel((s) => { const n = new Set(s); for (const r of visible) { if (allVisible) n.delete(r.fullName); else n.add(r.fullName); } return n; })}>{allVisible ? "표시된 것 모두 해제" : "표시된 것 모두 선택"}</button>
          </div>

          <div className="rows">
            {visible.length === 0 && <div className="empty small">조건에 맞는 저장소가 없습니다. 기간을 넓히거나 필터를 풀어보세요.</div>}
            {visible.map((r) => (
              <label key={r.fullName} className="pick-row">
                ...
                <div style={{ minWidth: 0 }}>
                  <div className="row" style={{ gap: 8, alignItems: "baseline" }}>
                    ...
                    {r.isPrivate && <span className="badge outline">비공개</span>}
                    {r.fork && <span className="badge outline">포크</span>}
                    {r.archived && <span className="badge outline">아카이브</span>}
                  </div>
                ...
                <div className="tiny muted" style={{ textAlign: "right", whiteSpace: "nowrap" }}>
                  ...
                  <div>{r.pushedAt ? `갱신 ${relTime(r.pushedAt)}` : "갱신 기록 없음"}</div>
                </div>
              </label>
            ))}
          </div>
