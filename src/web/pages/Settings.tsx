...
import type { Example, KeyStatus, SettingsView, Source } from "@shared/types";
...
export default function Settings() {
  ...
  const upsertSource = (p: Record<string, unknown>) => post("/sources", p);
  const removeSource = (p: { id: number }) => del(`/sources/${p.id}`);
  const { data: examples } = useResource<Example[]>("/examples", ["examples"]);
  const { data: keyStatus } = useResource<KeyStatus[]>("/keys", ["keys"]);
  const setActive = (p: { id: number; active: boolean }) => post(`/examples/${p.id}/active`, { active: p.active });
  const removeExample = (p: { id: number }) => del(`/examples/${p.id}`);
  const addExample = (p: Record<string, unknown>) => post("/examples", p);
  ...
  return (
    <>
      ...
      <h2>소스</h2>
      <div className="card">
        <p className="small muted">GitHub 조직/사용자 이름 또는 owner/repo. 쉼표로 구분. 서버의 GITHUB_TOKEN 환경변수가 필요합니다.</p>
        <div className="row">
        ...
        <div className="list" style={{ marginTop: 10 }}>
          {(sources ?? []).map((s) => (
            <div key={s.id} className="row between small">
              <span>
                <span className="badge">{s.kind}</span> {s.targets.join(", ")}
              ...
              <span className="toolbar">
                <button onClick={() => void upsertSource({ id: s.id, kind: s.kind, targets: s.targets, options: s.options, enabled: !s.enabled })}>{s.enabled ? "끄기" : "켜기"}</button>
                <button className="danger" onClick={() => void removeSource({ id: s.id })}>삭제</button>
              </span>
            </div>
          ))}
        </div>
      ...
      <div className="card">
        ...
        <button className="primary" onClick={() => void update({ draftThreshold: thresholds.draft, deferThreshold: thresholds.defer, bannedPhrases: banned.split("\n").map((s) => s.trim()).filter(Boolean) })}>저장</button>
      </div>
...
SOMUN_URL=${window.location.origin} SOMUN_TOKEN=<token> npm run agent-worker -- --cli ${llm.agentCli}`}</pre>
          </>
        ) : (
          <div className="row">
      ...
      <div className="card">
        ...
        <div className="list">
          {(examples ?? []).filter((e) => exChannel === "all" || e.channel === exChannel).map((e) => (
            <div key={e.id} className="row between small" style={{ alignItems: "flex-start" }}>
              <div style={{ flex: 1 }}>
              ...
              <span className="toolbar">
                <button onClick={() => void setActive({ id: e.id, active: !e.active })}>{e.active ? "끄기" : "켜기"}</button>
                <button className="danger" onClick={() => void removeExample({ id: e.id })}>삭제</button>
              </span>
            </div>
          ))}
        </div>
