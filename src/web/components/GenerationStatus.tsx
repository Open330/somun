  ...
  return <section className="generation-status" aria-label="생성 진행 상태">
    ...
    <ul className="generation-jobs" aria-live="polite">
      {jobs.map((job) => <li key={job.id}>
        <div className="row wrap"><span>{labels[job.kind]}{job.channel ? ` · ${CHANNEL_LABEL[job.channel]} ${job.lang?.toUpperCase()}` : ""}</span><span className={`badge ${job.status === "failed" ? "bad" : job.status === "done" ? "ok" : "outline"}`}>{job.status === "claimed" ? "진행 중" : job.status === "pending" ? job.executor === "local" ? "로컬 워커 대기" : "순서 대기" : job.status === "done" ? "완료" : "실패"}</span>{candidateId === undefined && <Link to={`/c/${job.candidateId}`}>{candidateTitles?.[job.candidateId] ?? "글감 보기"}</Link>}</div>
