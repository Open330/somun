...
export class ChunkBoundary extends Component<{ children: ReactNode; resetKey?: string }, { failed: boolean }> {
  ...
  render() {
    ...
    return <div className="state-panel error-state" role="alert"><span className="state-symbol" aria-hidden>!</span><h2>화면을 불러오지 못했습니다</h2><p>새 버전이 배포되었거나 연결이 끊겼을 수 있습니다. 새로고침하면 최신 화면을 받습니다.</p><button className="primary" onClick={() => window.location.reload()}>새로고침</button></div>;
  }
}
