...
export const toDraft = (r: typeof schema.drafts.$inferSelect): Draft => ({ id: r.id, candidateId: r.candidateId, channel: r.channel as Channel, version: r.version, title: r.title ?? undefined, body: r.body, mediaHint: r.mediaHint ?? undefined, lint: r.lint, status: r.status as Draft["status"], model: r.model, createdAt: r.createdAt, updatedAt: r.updatedAt });
export const toPublication = (r: typeof schema.publications.$inferSelect): Publication => ({ id: r.id, candidateId: r.candidateId, draftId: r.draftId ?? undefined, channel: r.channel as Channel, url: r.url, publishedAt: r.publishedAt, manualStats: r.manualStats ?? undefined });
