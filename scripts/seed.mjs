#!/usr/bin/env node
/**
 * seeds/*.json 과 seeds/README.md 의 실물 예시를 examples 테이블에 넣는다.
 *   CONVEX_URL=http://127.0.0.1:3210 node scripts/seed.mjs
 * 인증 배포에는 SOMUN_TOKEN(외부 JWT)을 함께 준다.
 */
import { ConvexHttpClient } from "convex/browser";
import { readFileSync } from "node:fs";
import { anyApi } from "convex/server";

const url = process.env.CONVEX_URL ?? process.env.VITE_CONVEX_URL ?? readEnvLocal("VITE_CONVEX_URL");
...
function readEnvLocal(key) {
  try {
    const m = new RegExp(`^${key}=(.*)$`, "m").exec(readFileSync(new URL("../.env.local", import.meta.url), "utf8"));
    return m?.[1]?.trim();
  } catch {
    return undefined;
  }
...
for (const f of ["hn_seeds.json", "hn_seeds_devtools.json"]) {
  const rows = JSON.parse(readFileSync(new URL(`../seeds/${f}`, import.meta.url), "utf8"));
  for (const r of rows) {
    if (!r.first_comment || r.first_comment.length < 120) continue;
    items.push({ channel: "show_hn", lang: "en", title: r.title, body: r.first_comment, note: `${r.points}p · ${r.hn}` });
  }
}
// X 영어 실물 두 개 (seeds/README.md)
items.push(
  { channel: "x_en", lang: "en", body: "Ghostty devlog video: I'm working on search! Here's 15 minutes of me explaining why it has taken so long, what is hard about it, the architecture we're going for, and the big progress I've made recently. Progress PR if you'd rather read stuff: https://github.com/ghostty-org/ghostty", note: "Mitchell Hashimoto" },
  { channel: "x_en", lang: "en", body: "PRs on OpenClaw are growing at an *impossible* rate. Worked all day yesterday and got like 600 commits in. It was 2700; now it's over 3100. I need AI that scans every PR and Issue and de-dupes.", note: "Peter Steinberger" },
);
// 한국어 형식 예시 (규칙 기반, seeds/README.md)
items.push(
  { channel: "x_ko", lang: "ko", body: "에이전트가 권한 확인에 걸려 멈춘 걸 30분 뒤에 아는 게 지겨워서 만들었습니다.\n\nmuxa: tmux 위에서 코딩 에이전트가 일하는 중인지, 기다리는 중인지 보여주고, 가장 오래 기다린 창으로 바로 점프합니다.\n\nRust, 릴리스 61회, 아직 Windows는 안 됩니다. https://github.com/Open330/muxa", note: "형식 예시" },
